...
var KNN = function () {
	...
	_createClass(KNN, [{
		key: "distance",
		value: function distance(arrayA, arrayB) {
			var sum = 0;
			arrayA.forEach(function (value, index) {
		...
		key: "updateMax",
		value: function updateMax(value, array) {
			var max = 0;
		...
		value: function predict(value) {
			...
			this.data.forEach(function (object) {
				var entry = {
					distance: _this.distance(value, object.value),
					id: object.id
				};

				if (closest.length < _this.topK) {
					closest.push(entry);
					maxDistance = _this.updateMax(maxDistance, closest);
			...
			closest.forEach(function (entry) {
				votes.push(entry.id);
			});

			var prediction = this.mode(votes);
		...
		key: "deleteClassData",
		value: function deleteClassData(id) {
			for (var index = this.data.length - 1; index >= 0; index -= 1) {
			...
			if (this.canChange) {
				if (this.counter < this.speed) {
					...
					if (this.type === 'clap' && this.frame === 2) {
						_main2.default.player.duck();
						if (_main2.default.clapSound) {
							_main2.default.clapSound.play();
						}
						this.triggered = true;
					}

					if (this.type === 'say' && this.frame === 2) {
						_main2.default.player.jump();
						if (_main2.default.ohSound) {
							_main2.default.ohSound.play();
						}
						this.triggered = true;
					}
				}
...
var Obstacle = function () {
	...
	_createClass(Obstacle, [{
		...
		value: function hittest() {
			var a = {
				x: _main2.default.player.x + _main2.default.player.offsetX,
				y: _main2.default.player.y + _main2.default.player.offsetY,
				height: _main2.default.player.height,
				width: _main2.default.player.width
			...
			for (var x = 0; x < xOverlap; x += 1) {
				for (var y = 0; y < yOverlap; y += 1) {
					if (!found) {
						var char = this.mask[x + baseX][y + baseY];
						if (char === 'x') {
							found = true;
						}
					}
		...
		value: function render(context) {
			...
			var test = this.intersectRect({
				...
				left: _main2.default.player.x + _main2.default.player.offsetX,
				top: _main2.default.player.y + _main2.default.player.offsetY,
				bottom: _main2.default.player.y + _main2.default.player.offsetY + _main2.default.player.height,
				right: _main2.default.player.x + _main2.default.player.width - _main2.default.player.offsetX
			});

			if (test) {
				// Intersecting boundingboxes, check pixels
				var pixelHit = this.hittest();
				if (pixelHit) {
					var event = new CustomEvent('hit', {
						detail: {
							obstacle: this.type
						}
					});
...
var ScoreBoard = function () {
	function ScoreBoard() {
		...
		this.value = null;
		this.numbers = [];
		this.numbers.push(new _Number2.default(0));
		this.numbers.push(new _Number2.default(0));
		this.numbers.push(new _Number2.default(0));
		this.numbers.push(new _Number2.default(0));
...
var Section = function () {
	function Section(x, noMonsters) {
		...
		var numObstacles = Math.floor(Math.random() * 2) + 1;
		// let numObstacles = 10
		var basePosition = this.x;
		...
		for (var index = 0; index < numObstacles; index += 1) {
			...
			// }
			var position = basePosition + Math.random() * 150;
			// let position = basePosition + 100
			if (position + 200 - this.x < this.width - 100) {
				var obstacle = new _Obstacle2.default(type, position);
				this.obstacles.push(obstacle);
